...
「主催」「会場」「参加費」などはページ上の見出しから拾いますが、
**見出しの後ろにコロンか改行が続く場合しか見出しとして扱いません。**

これは、同じ言葉が値の中に現れるためです。実際に次の取り違えが起きていました。

| 元の文字列 | 誤って拾っていた値 |
|---|---|
| `【クロスギルド主催】8/26(水) 参加費無料 BOX争奪戦`（大会名） | `】8/26(水) 参加費無料 BOX争奪戦` |
| `火蝶杯（公認オーガナイザーイベント）`（主催者名） | `イベント）` |

「オーガナイザー」は見出しの候補から外しました。Tonamelでは見出しとして使われず、
主催者名の中にだけ現れるためです。

### 一覧の値で詳細の値を上書きしない

一覧ページのカードには「主催」「会場」といった見出しがありません。
そのため一覧だけを取った実行では、これらの項目は取れないか、上のような取り違えが起きます。
これをそのまま保存すると、**実行のたびに表記が揺れます**（実際に起きていました）。

そこで、前回 詳細ページまで取れていた大会については、
一覧だけの実行では **大会名・日程・URL・画像だけを更新**し、
主催・会場・住所・参加費・定員は前回の値を残します。
大会名か日程が変わっていた場合は、その場で詳細ページも取り直して全体を合わせます。

取得元（`source`）は `list` → `api` → `detail` の方向にしか進みません。
一度 詳細まで取れた大会が、次の実行で「一覧だけ」に格下げされることはありません。
